...
def _stack_first_four_crops(crops):
    return torch.stack([fluore_to_tensor(crop) for crop in crops[:4]])


def _normalize_tensor_255_to_centered(x):
    return x.float().div(255).sub(0.5)
...
def main():
    ...
    if args.transform == 'four_crop':
        ...
        transform = transforms.Compose([
            ...
            transforms.Lambda(_stack_first_four_crops),
            transforms.Lambda(_normalize_tensor_255_to_centered)
            ])
